<? include 'include/context.php'; ?>
<? include 'include/header.php'; ?>
<? include 'include/menu.php'; ?>

<div class="main">
	<div class="center">
		<div class="content">
			<p class="meta">Hi, there!</p>
			<p>I’m Lucian and I live in <a href="https://en.wikipedia.org/wiki/Bucharest">Bucharest</a>, <a href="https://en.wikipedia.org/wiki/Romania">Romania</a>. I mostly programming backends these days, but I started as a designer and worked as frontend engineer before. I enjoy coding in Python (Django, Falcon), JavaScript (Vanilla, Mithril), PHP, Swift and SQL (Postgres, Oracle).</p>
			<p style="margin-top: 5px;">Designing user interfaces on a Mac remains my hobby. I’m striving for perfection and I pay attention to the tiniest details in everything I'm trying to achieve.<p>
		</div>
		<div class="content">
			<p class="meta">Contact me</p>
			<p class="link">Gmail</p>
			<p class="link">marin.lucian</p>
			<p class="link">Phone</p>
			<p class="link">+40 726 210 589</p>
		</div>
		<div class="content">
			<p class="meta">Almost social</p>
			<p class="link">
				<a href="https://dribbble.com/lucianmarin">Dribbble</a>
			</p>
			<p class="link">
				<a href="https://github.com/lucianmarin">GitHub</a>
			</p>
			<p class="link">
				<a href="https://instagram.com/lucianmarin/">Instagram</a>
			</p>
			<p class="link">
				<a href="https://ro.linkedin.com/in/marinlucian">LinkedIn</a>
			</p>
			<p class="link">
				<a href="https://soundcloud.com/lucianmarin/likes">SoundCloud</a>
			</p>
			<p class="link">
				<a href="https://twitter.com/lucianmarin">Twitter</a>
			</p>
		</div>
		<div class="content">
			<p class="meta">Tiny resume</p>
			<p class="link">2018&ndash;2019</p>
			<p class="link">Secureworks</p>
			<p class="link">&nbsp;</p>
			<p class="link">Amgen</p>
			<p class="link">2017</p>
			<p class="link">Romlogic</p>
			<p class="link">2015&ndash;2017</p>
			<p class="link">ChargeAds</p>
			<p class="link">2012&ndash;2013</p>
			<p class="link">DevLabel</p>
			<p class="link">2011</p>
			<p class="link">SourceFabric</p>
			<p class="link">2008</p>
			<p class="link">Sparktech</p>
			<p class="link">2006&ndash;2007</p>
			<p class="link">Firedive</p>
		</div>
	</div>
</div>

<? include 'include/footer.php'; ?>
